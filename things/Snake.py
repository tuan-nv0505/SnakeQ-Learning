...
class Snake:
    ...
    def get_state(self, food):
        head = self.get_head()
        next_positions = self.get_next_pos()
    ...
    def is_alive(self):
        ...
        # return not (self.is_collision_body(self.get_next_pos()[self.direction.value]) or self.is_collision_wall(
        #     self.get_next_pos()[self.direction.value]))
